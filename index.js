...
break
case 'menu':
vn = fs.readFileSync('./audio/menu.mp3')
mng = await getBuffer(`https://melcanz.com/kaneki?nama=${logoName}&apikey=rahasia`)
...
${simbol} ${prefix}setnamabot <namabot>
...
sendButImg(from, st, st2, mng, but, mek)
Lexxy.sendMessage(from, vn, audio, { quoted: mek, mimetype: 'audio/mp4', ptt:true })
break
case 'start':
const hky = speed();
const hby = speed() - hky
...
boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
delb = await Lexxy.downloadMediaMessage(boij)
fs.writeFileSync(`./gambar/manu.png`, delb)
reply('Sukses')
} else {
